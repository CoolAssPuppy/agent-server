...
function replaceSecrets(text) {
    ...
    for (const pattern of SECRET_PATTERNS) {
        sanitized = sanitized.replace(pattern, (_m, prefix) => {
            if (prefix)
...
export function sanitizeText(input, maxLength = 512) {
    const cleaned = input
...
export function sanitizePromptSuffix(input) {
...
export function sanitizeProgressEvent(event) {
    return {
        ...event,
