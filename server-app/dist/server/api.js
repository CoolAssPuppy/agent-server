...
import { z } from 'zod';
import { AuthFailureTracker, InMemoryRateLimiter, getClientIp, sanitizePromptSuffix, sanitizeStoredRun, sanitizeText, } from './security-utils.js';
const MAX_BODY_BYTES = 8_192;
const TriggerRunBodySchema = z.object({
    with: z.string().trim().max(4_000).optional(),
});
function isAuthorized(requestKey, expectedKey) {
    if (!requestKey)
        return false;
...
function extractApiKeyHeader(request) {
...
function parseOriginHost(originHeader) {
    try {
...
function isSameOriginRequest(request, expectedHost) {
    ...
    if (originHost === normalizedExpected)
        return true;
    if (isLoopbackHost(normalizedExpected) && isLoopbackHost(originHost))
        return true;
    return false;
}
