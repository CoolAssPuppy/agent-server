...
function createContext() {
  ...
  return { logger, context: { logger, agentId: 'daily-manuscript-review', runId: 'run-2' } };
}
...
describe('agent log read tool', () => {
  ...
  it('matches entries by message so an agent can ask for one kind of record', async () => {
    const { logger, context } = createContext();
    logger.append({ agentId: 'daily-manuscript-review', runId: 'run-1', message: 'Manuscript hash', data: { hash: 'aaa' } });
    logger.append({ agentId: 'daily-manuscript-review', runId: 'run-1', message: 'Run completed', source: 'server' });
  ...
  it('honors a limit and caps how much a run can pull back', async () => {
    ...
    for (let index = 0; index < 10; index += 1) {
      logger.append({ agentId: 'daily-manuscript-review', runId: 'run-1', message: `entry ${index}` });
    }
  ...
  it('never returns another agent entries', async () => {
    const { logger, context } = createContext();
    logger.append({ agentId: 'other-agent', runId: 'run-9', message: 'not yours' });
