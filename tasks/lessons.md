...
- A feature is done when the state it creates is visible in the app, not when the write path works. Pairing stored a credential with no way to show it; the security gate refused runs with one log line. Both shipped "working" and both read as broken. Before closing any feature, answer: what does the person see the next day without the terminal?
- Server JSON and Swift CodingKeys are one contract in two files with no test between them. Any new response model needs a golden-fixture decode test on the Swift side before it ships, or the mismatch is found by the user as "The data couldn't be read."
- Tests must not read the real ~/.agent-server. Machine state changes underneath them; pairing this Mac broke two config tests and printed a live credential into the failure output. Inject paths and pairing lookups.
- One fact, one source, every consumer. The security verdict reached the sidebar but not the agent page, so two screens told opposite stories about the same agent at the same moment. When adding a new fact to any view, grep for every other view that makes a claim the fact could contradict, and wire them all in the same change.
- User-facing copy states what to do, never how the machinery works. "Entering a new code replaces the credential it holds" is jargon; "To pair this Mac again, get a new code from Panel" is the sentence.
