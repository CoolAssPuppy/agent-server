...
- **StatusMonitor**: Connects to `ws://localhost:47821/ws` for real-time run events. Falls back to HTTP polling every 5 seconds if WebSocket disconnects. Reconnects automatically after 5 seconds. Uses `@Published` properties for reactive UI updates. On lifecycle events (`run_completed`, `run_failed`, `mcp_status`) it calls `poll()` and routes to `NotificationManager`, resolving `agentId → agent.name` from the already-published agents list. Routes `run_failed` with `code == "run_timeout"` to `notifyRunTimedOut` and `run_failed` without a timeout code to `notifyRunFailed`. Detects server restarts by comparing `/health` `started_at` values between polls and fires `notifyServerRestarted` on change. Does NOT notify on `run_started` (too noisy).
- **AgentServerClient**: HTTP client with `cancelRun(id:)` for aborting running agents via `POST /runs/:id/cancel`.
- **ServerProcessManager**: Discovers the server in three locations: `AGENT_SERVER_LOCATION` override (UserDefaults or `.env`), bundled Resources (`Contents/Resources/dist/cli.js`), or a sibling `agent-server/` directory next to the `.app`. When spawning the Node server it exports `AGENT_SERVER_EVENTKIT_BIN` pointing at the bundled `Contents/Helpers/agent-server-eventkit` so the Node executor can auto-inject the eventkit MCP server. Does not run `npm install` at runtime; production `node_modules/` is pre-staged at build time (see "Bundled server" below).
- **NotificationManager**: Wraps `UNUserNotificationCenter`. Requests authorization on launch. Every post call declares a `NotificationCategory` (`.systemEvent` or `.agentOutput`) and checks `NotificationPreferences.shared.shouldPost(category)` before emitting. Uses the bundled `agent-server-notification.caf` chime from `Contents/Resources/`. Silently no-ops if authorization is denied or the relevant toggle is off. See "Notification preferences" below for the two-tier model and event-to-category routing.
...
| Toggle | Stored key | Category gated | Events |
|---|---|---|---|
| Enable notifications | `notifications.enabled` | `.systemEvent` (and master for tier 2) | MCP needs auth, run timeout, server restart |
| Also notify for agent output | `notifications.includeAgentOutput` | `.agentOutput` | Run completed, run failed (non-timeout) |

Both default ON. `shouldPost(.systemEvent)` requires only the master; `shouldPost(.agentOutput)` requires both. `NotificationCategory` lives in the core test module so the gate matrix is unit-testable. UI settings live in `SettingsDrawer.generalCard` (primary) and `SettingsTabView` (legacy sheet path); both bind to `NotificationPreferences.shared`.
...
Building the `AgentServer` scheme runs `preBuildScripts` first (stages production `node_modules/` in `.build-cache/server-bundle/` via `npm ci --omit=dev`), then builds `AgentServerEventKit` (target dependency), then compiles Swift sources, copies resources into the bundle (`dist/`, `package.json`, `node_modules/`), and finally runs the post-build script that embeds the helper at `Contents/Helpers/agent-server-eventkit`. First build takes longer due to `npm ci`; subsequent builds skip it based on lock-file hash. To build only the helper for debugging: `xcodebuild -scheme AgentServerEventKit build`. To sanity-check the helper binary directly: `echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' | /path/to/agent-server-eventkit`.
