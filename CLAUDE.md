...
- **StatusMonitor**: Connects to `ws://localhost:47821/ws` for real-time run events. Falls back to HTTP polling every 5 seconds if WebSocket disconnects. Reconnects automatically after 5 seconds. Uses `@Published` properties for reactive UI updates. On lifecycle events (`run_started`, `run_completed`, `run_failed`) it both calls `poll()` and fires a native notification via `NotificationManager`, resolving `agentId → agent.name` from the already-published agents list. Detects server restarts by comparing `/health` `started_at` values between polls.
...
- **NotificationManager**: Wraps `UNUserNotificationCenter`. Requests authorization on launch. Three notification types: run started, run completed (body = Agent SDK's final result text, truncated to 240 chars), run failed (body = error or final message). Silently no-ops if authorization is not granted. No per-agent configuration; notifications fire for all runs.
